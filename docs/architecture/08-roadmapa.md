...
| **AI-3** ✔ — zatwierdzane wykonywanie | narzędzia akcji `send_to_terminal` (wpisanie/uruchomienie komendy — działa też dla UART, bo pisze do aktywnej sesji) i `write_file`; KAŻDA akcja przechodzi przez bramkę zatwierdzania w czacie (inline „Zatwierdź/Odrzuć" z podglądem), a decyzja i skutek trafiają do dziennika audytowego `logs/ai-audit.log`. Bez zgody nic się nie wykona |
...
| **AI-5** ✔ — agent wieloetapowy | pętla agenta wydzielona do testowalnego runnera (`renderer/ai/agent.ts`) z wstrzykiwanymi zależnościami: sekwencja tur model↔narzędzia pod kontrolą limitu kroków, limitu akcji, budżetu czasu, twardego przerwania (AbortSignal) oraz ponawiania z backoffem przy błędach przejściowych (4xx bez retry). Testy jednostkowe sterowania 20/20 |
| **AI-6** ✔ — narzędzia wtyczek | wtyczki wystawiają narzędzia AI przez `contributes.tools` (JSON Schema wejścia) pod uprawnieniem `ai.tools`; host wywołuje je request/response po `callId` (`context.tools.registerTool` zwraca tekst dla modelu), a `risky:true` przechodzi przez tę samą bramkę zgody i audyt co akcje AI-3. Nazwy namespace'owane, scalane z wbudowanymi w toolset. Przykładowa wtyczka `toolbox` + [dokumentacja SDK](../plugin-api/03-narzedzia-agenta.md). Wielopoziomowe ryzyko → AI-7 |
| **AI-7** ✔ — kontrolowana autonomia | trwała **polityka agenta** (limity kroków, akcji, czasu i **budżet tokenów = limit kosztów**) w main, edytowalna w panelu; dostawcy raportują zużycie tokenów (OpenAI `include_usage`, Anthropic `message_start`/`message_delta`), runner egzekwuje budżet (status `budget`) i pokazuje licznik; **pełny rejestr działań** — każde wywołanie narzędzia (odczyt i akcja) trafia do `ai-audit.log`, podgląd z czatu. Wielopoziomowe profile ryzyka urządzeń/zaufane sesje — dalsze zaostrzenie na bazie tej warstwy |
