...
### Aktualizacja po etapie polishu UI: własne przyciski-kółka zamiast WCO

Na wyraźne życzenie użytkownika przyciski okna to teraz **własne kolorowe kółka** (styl
„traffic lights" w palecie: bursztyn/zielony/koral), rysowane w pasku tytułu. To odwraca
poprzednią decyzję: rezygnujemy z natywnego Window Controls Overlay, a **z nim ze Snap
Layouts**, na rzecz spójnej estetyki. Okno używa `titleBarStyle: 'hidden'` **bez**
`titleBarOverlay`; sterowanie idzie z powrotem przez IPC (`window:minimize/maximizeToggle/
close/isMaximized` + zdarzenie `maximizedChanged`). Resize krawędziami działa dalej
(`thickFrame`), okno pozostaje przesuwalne przez region `-webkit-app-region: drag`.

Świadomy koszt: brak flyoutu Snap Layouts po najechaniu na maksymalizację. Gdyby okazał się
potrzebny, wracamy do `titleBarOverlay` (przyciski przestają być kółkami).
